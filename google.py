...
def main():
    base_url = "https://so2.cljtscd.com/scholar?start="  #要爬取的网页链接
    # 1.爬取网页
    headers = {
    ...
    page_info=getData(base_url,search_word,page_num)
    
    # write_to_excel(book_path,page_info,sheet_name,WriteMode.APPEND)
    write_to_excel(book_path,page_info,search_word)
...
# 爬取网页的内容,并解析,填入列表
def getData(baseurl,search_word,page_num,one_page_num=10): # 默认一页10篇文献
    datalist = [] 
...
def write_to_excel(book_path, info,search_word,mode=WriteMode.OVERWRITE):
    sheet = None
